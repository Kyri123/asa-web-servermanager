...
export class Logger<T extends CustomLogs> {
	...
	private shouldLog(logLevel: LogLevel): boolean {
		...
		return (logLevel >= this.options.minLevel && logLevel <= this.options.maxLevel) || logLevel === LogLevel.FATAL;
	}
	...
	private formatMessage(name: keyof typeof this.loggerNames, logLevel: LogLevel, message: any, ...optionalParams: any[]): any {
		const datetime = moment().format('YYYY-MM-DD HH:mm:ss');
		let msg = this.options.format;
		msg = msg.replace('{datetime}', datetime);
		msg = msg.replace('{level}', this.wrapInColor(`[${String(logLevel)}]`, this.logLevelColor(logLevel)));
		msg = msg.replace('{level_name}', this.wrapInColor(`[${this.getLogLevelName(logLevel).toUpperCase()}]`, this.logLevelColor(logLevel)));
		msg = msg.replace('{logger_name}', this.wrapInColor(String(name).replace(/_/g, ' ').toUpperCase(), this.logLevelColor(logLevel)));
	...
	public log(name: keyof typeof this.loggerNames, message: any, ...optionalParams: any[]): void {
		...
		if (this.shouldLog(logLevel)) {
			this.consoleFn(logLevel)(...this.formatMessage(name, logLevel, message, ...optionalParams));
